...
import { scriptDeleteDrink, scriptEditDrink } from "../../store/index";
...
const Drink = () => {
  ...
  const destroy = async () => {
    await dispatch(scriptDeleteDrink(drink));
  ...
  const save = async (ev) => {
    ev.preventDefault();
    try {
      await dispatch(scriptEditDrink({ name, category, imageUrl, description, price, id }));
      navigate("./admin/drinks");
    } catch (err) {
      setErrors(err);
      console.log(err);
  ...
  return (
    <div style={{ margin: "auto", maxWidth: "80%", fontSize: "1.4rem", padding: "1rem" }}>
      ...
      <h5>
        <Link to={"/admin/drink"}>Back to Drink List</Link>
      </h5>
      <h5>
