...
import { scriptCreateReview, scriptDeleteReview } from "../store/index";
import { useNavigate } from "react-router-dom";

const Review = ({ match }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { scriptAuth, scriptReviews, scriptMerches, scriptDrinks } = useSelector((state) => state);
  ...
  const create = async (ev) => {
    ev.preventDefault();
    try {
      await dispatch(scriptCreateReview({ subject, description, rating, drinkId, merchId }));
      setSubject("");
      setDescription("");
      setRating("");
      setDrinkId("");
  ...
  const destroy = async (review) => {
    await dispatch(scriptDeleteReview(review));
  };
    ...
    <div>
      ...
      <div className="formBlock">
        <form className="form" onSubmit={create}>
          ...
          <button className="formButton" disabled={!subject || !description || !rating}>
            Submit Your Review
          </button>
        ...
        <div className="formBlock">
          <div className="form">
            <ul>
              {scriptReviews
                .filter((review) => review.userId === scriptAuth.id)
                .map((review) => {
                  return (
                    <li key={review.id}>
                      {scriptAuth.id === review.userId ? "Your Review" : ""}
                      <br />
