...
import { scriptCreateDrink } from "../../store/index";
...
const CreateDrink = () => {
  ...
  const create = async (ev) => {
    ev.preventDefault();
    await dispatch(scriptCreateDrink({ name, category, imageURL, hasImage, size, description }));
    ...
    <div style={{ margin: "auto", maxWidth: "80%", fontSize: "1.4rem", padding: "1rem" }}>
      ...
      <form onSubmit={create}>
        ...
        <input
          ...
          value={imageURL}
          onChange={(ev) => {
            setImageURL(ev.target.value), useEffect();
          }}
        />
