...
import { scriptDeleteMerch, scriptEditMerch } from "../../store/index";
...
const AdminMerchesMerch = () => {
  ...
  const destroy = async () => {
    await dispatch(scriptDeleteMerch(merch));
  ...
  const save = async (ev) => {
    ev.preventDefault();
    try {
      await dispatch(scriptEditMerch({ name, category, imageUrl, description, price, id }));
      navigate("./admin/merches");
    } catch (err) {
      setErrors(err);
      console.log(err);
    ...
    <div style={{ margin: "auto", maxWidth: "80%", fontSize: "1.4rem", padding: "1rem" }}>
      ...
      <form onSubmit={save} style={{ margin: "auto", maxWidth: "80%", minWidth: "350px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-around" }}>
          ...
          <button style={{ width: "200px" }} onClick={destroy}>
            {" "}
            delete merch??{" "}
          </button>
        </div>
      ...
      <h5>
        <Link to={"/admin/merch"}>Back to Merch List</Link>
      </h5>
      <h5>
