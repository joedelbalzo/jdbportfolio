...
const MerchShirts = () => {
  const { scriptMerches } = useSelector((state) => state);
  ...
  const shirts = scriptMerches.filter((m) => m.category == "shirt");

  const _moreDetails = (merch) => {
    navigate(`/merch/${merch.id}`);
  };
  const _addToCart = (merch, quantity) => {
    dispatch(scriptAddToCart(merch, quantity));
  };
    ...
    return (
      ...
      >
        ...
        <CardActionArea sx={{ textAlign: "center", alignItems: "center" }}>
          <Button component="span" onClick={(ev) => _moreDetails(merch)} size="small">
            More Details
          </Button>
          <CardActions>
            <Box sx={{ minWidth: 130 }}>
            ...
            <Button component="span" size="small" onClick={() => _addToCart(merch, quantity)}>
              Add To Cart
            </Button>
          </CardActions>
        </CardActionArea>
