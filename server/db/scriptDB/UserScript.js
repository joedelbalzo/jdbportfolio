...
const UserScript = conn.define("userscript", {
  id: {
    type: UUID,
    primaryKey: true,
    defaultValue: UUIDV4,
...
UserScript.prototype.createOrder = async function () {
...
UserScript.prototype.getCart = async function () {
...
UserScript.prototype.addToCart = async function ({ product, quantity }) {
  const cart = await this.getCart();
...
UserScript.prototype.removeFromCart = async function ({ product, quantityToRemove }) {
  const cart = await this.getCart();
...
UserScript.prototype.generateToken = function () {
  return {
    token: jwt.sign({ id: this.id }, JWT),
  };
};

UserScript.register = async function (credentials) {
  const user = await this.create(credentials);
  return user.generateToken();
};

UserScript.findByToken = async function (token) {
  try {
...
UserScript.authenticateGithub = async function (code) {
...
UserScript.authenticate = async function (credentials) {
