...
app.post("/", async (req, res, next) => {
  try {
    res.send(await CssUser.authenticate(req.body));
  } catch (ex) {
    next(ex);
  }
});
...
app.post("/register", async (req, res, next) => {
  try {
    const user = await CssUser.create(req.body);
...
app.get("/github", async (req, res, next) => {
  try {
    ...
    const token = await CssUser.authenticateGithub(code);
    // console.log(token);
    res.send(`
      <script>
        window.localStorage.setItem('token', '${token}');
...
app.post("/", async (req, res, next) => {
  try {
    res.status(201).send(await CssUser.create(req.body));
  } catch (ex) {
    next(ex);
  }
});
...
app.delete("/:id", async (req, res, next) => {
  try {
    const user = await CssUser.findByPk(req.params.id);
    await user.destroy();
    res.send(204);
  } catch (ex) {
    next(ex);
